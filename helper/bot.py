...
from pyrogram.errors import SessionPasswordNeeded, AuthKeyInvalid, UserDeactivated, SessionRevoked, FloodWait
import functools
import socket
...
def connection_retry(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        for i in range(retries):
            ...
            except (socket.timeout, socket.error, ConnectionError, ConnectionResetError) as e:
                if i == retries - 1:
                    print(f"❌ [SOCKET ERROR] {args[1] if len(args) > 1 else 'Unknown'}: {e}")
                    raise
                await asyncio.sleep(2 * (i + 1))
            except FloodWait as e:
                print(f"⏳ [FLOOD] Waiting {e.value} seconds...")
...
class HelperManager:
    def __init__(self):
        self.bot = Client("helper_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workdir="database")
    ...
    async def start_all_userbots(self):
        ...
        if expired_sessions:
            print(f"🗑️ Membersihkan {len(expired_sessions)} session kadaluarsa.")
            conn.commit()
        
        conn.close()

        if to_boot:
            print(f"🚀 Memulai booting {len(to_boot)} userbot secara paralel...")
            
            # Start in batches (Smaller for Termux)
            is_termux = os.environ.get("IS_TERMUX") == "1"
            batch_size = 10 if is_termux else 25
            for i in range(0, len(to_boot), batch_size):
                batch = to_boot[i:i + batch_size]
                tasks = [self.boot_userbot(*data) for data in batch]
                results = await asyncio.gather(*tasks, return_exceptions=True)
                
                success = sum(1 for r in results if r is True)
                failed = len(batch) - success
                print(f"✅ Batch {i//batch_size + 1}: {success} Berhasil, {failed} Gagal.")
    ...
    async def boot_userbot(self, phone, aid, ahash, sess, bot_token=None):
        try:
            ...
            return True
        except (AuthKeyInvalid, UserDeactivated, SessionRevoked):
            print(f"❌ Session {phone} tidak valid (Revoked/Deactivated)")
            return False
        except Exception as e:
            if "socket" in str(e).lower(): raise # Let decorator handle retry
