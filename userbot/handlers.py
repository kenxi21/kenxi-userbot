...
async def forward_single_message(client, target_chat, source_chat, source_msg_id, semaphore, delay, user_id):
    """Forward a single message with timeout and semaphore control"""
    async with semaphore:
        ...
        try:
            ...
            try:
                ...
                print(f"[ERROR] Retry after FloodWait failed: {retry_err}")
...
async def fwd_handler(client, message):
    ...
    status_msg = await message.edit(f"🔄 **Memulai forward {count}x dengan delay {delay}s...**")
    await asyncio.sleep(1)
    
    spam_active[user_id] = True
    bot_semaphore = asyncio.Semaphore(CONCURRENT_FORWARDS_PER_BOT)
    
    tasks = []
    for i in range(count):
        task = forward_single_message(
            client, target_chat, source_chat, 
            source_msg_id, bot_semaphore, delay, user_id
        )
        tasks.append(task)
    ...
    for i, result_coro in enumerate(asyncio.as_completed(tasks), 1):
        ...
        if i % PROGRESS_UPDATE_INTERVAL == 0 or i == count:
            try:
                await status_msg.edit(
                    f"📡 **Forwarding...** ({i}/{count})\n"
                    f"✅ Berhasil: {success}\n"
                    f"❌ Gagal: {failed}"
                )
            except Exception:
    ...
    status_text = "✅ **Auto-Forward Selesai**" if not stopped else "🛑 **Auto-Forward Dihentikan**"
    await send_log(
        client,
        f"{status_text}\n"
        f"**Chat:** {chat_title}\n"
