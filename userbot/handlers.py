...
def connection_retry(max_retries=3):
    def decorator(func):
        @wraps(func)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f"[RETRY] Socket error, retrying in {wait_time}s... ({attempt + 1}/{max_retries})")
...
async def broadcast_worker(client, message, target_list, is_copy=False):
    ...
    for idx, chat_id in enumerate(target_list, 1):
        ...
        try:
            ...
            print(f"[WARN] Socket error in broadcast to {chat_id}: {e}")
