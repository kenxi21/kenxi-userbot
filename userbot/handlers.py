...
CONCURRENT_FORWARDS_PER_BOT = 5
FORWARD_TIMEOUT = 30
PROGRESS_UPDATE_INTERVAL = 10
MAX_FLOOD_WAIT_SLEEP = 300
MAX_SPAM_COUNT = 1000
MIN_SPAM_DELAY = 2    
...
# Global semaphore to limit concurrent high-volume operations across all bots
# This prevents 1000+ accounts from hitting the network/OS handles at the exact same time
global_broadcast_semaphore = asyncio.Semaphore(100)

def connection_retry(max_retries=5):
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        # Silent retry to keep logs clean for massive account usage
                        await asyncio.sleep(wait_time)
                    else:
                        break
                except RPCError as e:
                    last_error = e
                    # Specifically handle transient RPC errors if needed, otherwise re-raise
                    if "PEER_ID_INVALID" in str(e) or "CONNECTION_LOSS" in str(e):
                        if attempt < max_retries - 1:
            ...
            if last_error:
                print(f"[RETRY_FAILED] {func.__name__} after {max_retries} attempts: {last_error}")
            return None
        return wrapper
    return decorator
...
@connection_retry(max_retries=3)
async def worker_spam(client, chat_id, text, user_id):
    try:
        ...
        if e.value > MAX_FLOOD_WAIT_SLEEP:
            spam_active[user_id] = False
            await send_log(client, 
                f"🛑 **FloodWait Ekstrem: {e.value}s**\n"
                "Spam dihentikan otomatis untuk keamanan akun."
            )
            return False
        ...
        if e.value > FLOOD_WAIT_THRESHOLD:
            await send_log(client, 
                ...
                "Menunggu (bisa di-stop dengan .stop)..."
            )
            
            for _ in range(int(e.value)):
...
async def dspam_handler(client, message):
    ...
    if count > MAX_SPAM_COUNT:
        return await message.edit(
            f"❌ **Limit maksimal {MAX_SPAM_COUNT} pesan**\n"
            "Untuk keamanan akun Telegram."
        )
    
    if base_delay < MIN_SPAM_DELAY:
        return await message.edit(
            f"❌ **Delay minimal {MIN_SPAM_DELAY} detik**\n"
            "Untuk mencegah FloodWait."
        )
    ...
    if count > 500:
        warning = await message.reply(
            f"⚠️ **WARNING: Spam {count}x**\n"
            "Jumlah spam sangat tinggi, risk FloodWait besar.\n"
            "Gunakan `.stop` jika bot tidak responsif."
        )
    ...
    for i in range(count):
        ...
        res = await worker_spam(client, chat_id, spam_text, user_id)
        if res:
            success_count += 1
        elif not spam_active.get(user_id):
            break
        ...
        if spam_active.get(user_id):
            remaining = total_sleep - int(total_sleep)
            if remaining > 0:
                await asyncio.sleep(remaining)

    await send_log(
        client,
        f"✅ **DSpam Selesai**\n**Chat:** {chat_title}\n**Total Berhasil:** {success_count}/{count}"
    )

@connection_retry(max_retries=5)
async def fwd_handler(client, message):
    if len(message.command) < 3:
        return await message.edit(
            "❌ **Format:** `.fwd [delay] [jumlah] [link pesan]`\n"
            "atau balas pesan dengan `.fwd [delay] [jumlah]`\n\n"
            "**Contoh:**\n"
            "`.fwd 2 10` (balas ke pesan)\n"
            "`.fwd 2 10 https://t.me/c/1234/567`"
        )
    ...
    try:
        ...
        return await message.edit("❌ Delay dan jumlah harus angka!")
    ...
    if message.reply_to_message:
        ...
        msg_link = message.command[3]
        try:
            if '/c/' in msg_link:
                parts = msg_link.split('/c/')[1].split('/')
                source_chat = int('-100' + parts[0])
                source_msg_id = int(parts[1])
            elif 't.me/' in msg_link:
                parts = msg_link.split('t.me/')[1].split('/')
                source_chat = parts[0] if parts[0].startswith('@') else '@' + parts[0]
                source_msg_id = int(parts[1])
            else:
                return await message.edit("❌ Format link tidak valid!")
        except Exception as e:
            return await message.edit(f"❌ Gagal parse link: {e}")
    else:
        return await message.edit("❌ Balas ke pesan atau berikan link pesan!")
    
    chat_title = message.chat.title or "Chat"
    user_id = client.me.id
    target_chat = message.chat.id
    
    await message.edit(f"🔄 **Memulai forward {count}x dengan delay {delay}s...**")
    await asyncio.sleep(1)
    await message.delete()
    ...
    for i in range(count):
        ...
        try:
            await asyncio.wait_for(
                client.forward_messages(target_chat, source_chat, source_msg_id),
            ...
            await asyncio.sleep(e.value + 1)
            try:
                await client.forward_messages(target_chat, source_chat, source_msg_id)
                success += 1
            except:
                failed += 1
        except asyncio.TimeoutError:
            print(f"[ERROR] Forward timeout after {FORWARD_TIMEOUT}s")
            failed += 1
        except Exception as e:
...
async def broadcast_worker(client, message, target_list, is_copy=False):
    ...
    async with global_broadcast_semaphore:
        for idx, chat_id in enumerate(target_list, 1):
            if chat_id in blacklist:
                continue

            try:
                # Optimized delay for mass accounts
                ...
                # Dynamic wait based on severity
                wait_t = e.value + (5 if e.value < 60 else 30)
                ...
                # Silent retry logic for stability
                await asyncio.sleep(2)
                try:
                    if is_copy and msg_to_copy:
                        await msg_to_copy.copy(chat_id)
...
async def anti_view_once_watcher(client, message):
    try:
        ...
        if message.photo and hasattr(message.photo, 'ttl_seconds') and message.photo.ttl_seconds:
            ...
            is_view_once = True
        elif message.video_note and hasattr(message.video_note, 'ttl_seconds') and message.video_note.ttl_seconds:
            is_view_once = True
...
def install_ubot_handlers(ubot):
    ...
    ubot.add_handler(m(fwd_handler, filters.command("fwd", prefixes=".") & me))
