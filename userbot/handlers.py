...
from userbot.creating import dl_handler, kang_handler, stiker_handler, toimg_handler
...
def load_settings(user_id):
    path = f"database/config_{user_id}.json"
    ...
    try:
        ...
        print(f"[ERROR] Failed to load settings for user {user_id}: {e}")
        return {
            "pesan": DEFAULT_AUTOREPLY_MSG,
            "aktif": True,
            "pay_text": DEFAULT_SETPAY_MSG,
...
def save_settings(user_id, data):
    try:
        with open(f"database/config_{user_id}.json", "w") as f:
...
async def stop_autoreply_handler(client, message):
    ...
    if user_id == my_id:
        notification = await message.edit("❌ **Tidak bisa stop diri sendiri!**")
        await asyncio.sleep(QUICK_DELETE_DELAY)
        try:
            await notification.delete()
        except Exception:
    ...
    if my_id not in stopped_users:
        stopped_users[my_id] = {}
    
    stopped_users[my_id][user_id] = user_name

    if my_id in reply_counts and user_id in reply_counts[my_id]:
        del reply_counts[my_id][user_id]
...
async def unstop_autoreply_handler(client, message):
    ...
    if user_id == my_id:
        notification = await message.edit("❌ **Tidak bisa unstop diri sendiri!**")
        await asyncio.sleep(QUICK_DELETE_DELAY)
        try:
            await notification.delete()
        except Exception:
    ...
    if user_id in stopped_users.get(my_id, {}):
        del stopped_users[my_id][user_id]
        notification = await message.edit(
...
async def list_stopped_pm_handler(client, message):
    my_id = client.me.id
    my_stopped = stopped_users.get(my_id, {})
    ...
    for idx, (user_id, user_name) in enumerate(my_stopped.items(), 1):
        text += f"{idx}. **{user_name}**\n   └ ID: `{user_id}`\n\n"

    text += f"━━━━━━━━━━━━━━━━━━\n📊 Total: {len(my_stopped)} user"
...
async def clear_stopped_pm_handler(client, message):
    my_id = client.me.id
    my_stopped = stopped_users.get(my_id, {})
    count = len(my_stopped)
    
    if my_id in stopped_users:
        stopped_users[my_id].clear()
...
async def autoreply_handler(client, message):
    ...
    if my_id not in stopped_users:
        stopped_users[my_id] = {}
    if uid in stopped_users.get(my_id, {}):
        return
    ...
    if my_id not in reply_counts:
        reply_counts[my_id] = {}
    if uid not in reply_counts[my_id]:
        reply_counts[my_id][uid] = 0

    if reply_counts[my_id][uid] < AUTOREPLY_MAX_COUNT:
        try:
            await client.send_chat_action(message.chat.id, ChatAction.TYPING)
            await asyncio.sleep(1.5)
            await message.reply(settings.get("pesan", DEFAULT_AUTOREPLY_MSG))
            reply_counts[my_id][uid] += 1
        except Exception as e:
...
async def set_on(client, message):
    ...
    if my_id in stopped_users:
        stopped_users[my_id].clear()
    if my_id in reply_counts:
        reply_counts[my_id].clear()
    
    save_settings(my_id, settings)
...
def install_ubot_handlers(ubot):
    ...
    ubot.add_handler(m(kang_handler, filters.command("kang", prefixes=".") & me))
    ubot.add_handler(m(stiker_handler, filters.command(["s", "stiker"], prefixes=".") & me))
