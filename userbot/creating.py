...
async def stiker_handler(client, message):
    ...
    try:
        ...
        sticker_path = f"s_{message.id}.webp"
        
        if replied.sticker:
            # Jika sudah stiker, langsung kirim ulang saja agar jadi stiker (bukan doc)
            await client.send_sticker(message.chat.id, path)
            await message.delete()
            return

        # Proses image ke webp stiker
        img = Image.open(path)
        if img.mode != "RGBA":
        ...
        if 'path' in locals() and path and os.path.exists(path):
            os.remove(path)
        if 'sticker_path' in locals() and os.path.exists(sticker_path):
...
async def kang_handler(client, message):
    ...
    await message.edit("🎨 Menambahkan ke pack...")
